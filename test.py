...
if if_cross_validation:
    ...
    for i in range(4):
        best_lr, best_reg = cross_validation(k_fold, lr_set, reg_set, model, cross_entropy, AL_weight, epochs,
                         batch_size = batch_size, weight_sharing = if_weight_sharing[i], auxiliary_loss = if_auxiliary_loss[i])
        print(f"For weight_sharing = {if_weight_sharing[i]} and auxiliary_loss = {if_auxiliary_loss[i]}")
        print(f"best_lr = {best_lr}, best_reg = {best_reg}")
...
# hyperparameters
lr = [0.001, 0.001, 0.01, 0.01]  # learning rate
reg = [0.2, 0.1, 0.1, 0.1]  # weight decay factor (for l2 regularization)
